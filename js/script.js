...
faqs.forEach((faq) => {
  var faqTemplate = faqContainer
    .querySelector(".faq_template")
...
features.forEach((feature) => {
  var featureTemplate = document
    .querySelector(".feature_template")
    .content.cloneNode(true);
...
pricingPlans.forEach((plan) => {
  var priceTemplate = document
    .querySelector(".pricing_template")
    .content.cloneNode(true);
  ...
  if (plan.emphasized) card.classList.add("price-emp");
  ...
  if (plan.features)
    plan.features.forEach((feature) => {
      ...
      i++;
      var priceFeatureTemplate = priceTemplate
        .querySelector(".price_feature_template")
        .content.cloneNode(true);
      priceFeatureTemplate.querySelector(".price_feature").innerText =
        feature.featureName;
      priceFeatureTemplate.querySelector(".price_value").innerText =
        feature.featureValue;
      priceTemplate
        .querySelector(".price_features_grid")
        .append(priceFeatureTemplate);
    });
  i = 0;
  if (plan.feature)
    plan.feature.forEach((feature) => {
      ...
      i++;
      var priceFeatureTemplate = priceTemplate
        .querySelector(".price_feature_template_single")
        .content.cloneNode(true);
  ...
  priceTemplate.querySelector(
    ".price_footer"
  ).id = `razorpayButtonDestination${plan.id}`;
...
var moveRazorpayButtons = (source, destination) => {
